...
import org.kablink.teaming.gwt.client.rpc.shared.GetPhotoAlbumDisplayDataCmd;
...
public class PhotoAlbumFolderView extends FolderViewBase
	implements
...
{
	public final static boolean SHOW_GWT_PHOTO_ALBUM	= false;	//! DRF (20150318):  Leave false on checkin until it's all working.

	private List<HandlerRegistration>				m_registeredEventHandlers;	// Event handlers that are currently registered.
	private PhotoAlbumDisplayDataRpcResponseData	m_photoAlbumDisplayData;	// The photo album display data read from the server.
	...
	protected boolean allowNextPrevOnEntryView() {
		// By default, the photo album folder doesn't allow
		// next/previous.
		return false;
	}
	...
	protected boolean includePanel(FolderPanels folderPanel) {
		// In the photo album folder view, we show the default panels.
		boolean reply;
		switch (folderPanel) {
	...
	/*
	 * Asynchronously loads the photo album display data.
	 */
	...
	/*
	 * Synchronously loads the photo album display data.
	 */
	private void loadPart1Now() {
		GetPhotoAlbumDisplayDataCmd cmd = new GetPhotoAlbumDisplayDataCmd(getFolderInfo().getBinderIdAsLong());
		GwtClientHelper.executeCommand(cmd, new AsyncCallback<VibeRpcResponse>() {
			@Override
			public void onFailure(Throwable caught) {
				// Handle the failure...
				String error = m_messages.rpcFailure_GetPhotoAlbumDisplayData();
				GwtClientHelper.handleGwtRPCFailure(caught, error);
			}

			@Override
			public void onSuccess(VibeRpcResponse result) {
				m_photoAlbumDisplayData = ((PhotoAlbumDisplayDataRpcResponseData) result.getResponseData());
				populateViewAsync();
			}			
		});
	}
	...
	/**
	 * Called when the photo album folder view is attached.
	 * 
...
//!		...this needs to be implemented...
	...
	/**
	 * Called when the photo album folder view is detached.
	 * 
...
//!			...this needs to be implemented...
			...
			GwtClientHelper.deferredAlert("PhotoAlbumFolderView.onQuickFilter():  ...this needs to be implemented...");
		}
	}

	/*
	 * Asynchronously populates the the photo album view.
	 */
	...
	/*
	 * Synchronously populates the the photo album view.
	 */
	private void populateViewNow() {
//!		...this needs to be implemented...
		getFlowPanel().add(new Label("PhotoAlbumFolderView():  ...this needs to be implemented..."));
		viewReady();
	}
...
//!		...this needs to be implemented...
	...
	/*
	 * Asynchronously sets the size of the photo album based on its
	...
	public void viewComplete() {
		// Tell the photo album to resize itself now that it can
	...
	/* the photo album folder view and perform some operation on it. */
	/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
	
	/**
	 * Loads the PhotoAlbumFolderView split point and returns an
	...
	public static void createAsync(final BinderInfo folderInfo, final UIObject parent, final ViewReady viewReady, final ViewClient vClient) {
		GWT.runAsync(PhotoAlbumFolderView.class, new RunAsyncCallback() {			
			...
			public void onFailure(Throwable reason) {
				Window.alert(m_messages.codeSplitFailure_PhotoAlbumFolderView());
