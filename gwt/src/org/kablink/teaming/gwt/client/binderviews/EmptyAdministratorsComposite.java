...
/**
 * Class that holds the widget displayed for an empty administrators
 * view.
 * 
 * @author drfoster@novell.com
 */
public class EmptyAdministratorsComposite extends ResizeComposite {
	 ...
	 * @param ct
	 */
	public EmptyAdministratorsComposite() {
	...
	private void createContent() {
		...
		m_rootPanel.add(fp);
		Image i = GwtClientHelper.buildImage(m_filrImages.admins_transparent_72().getSafeUri().asString());
		fp.add(i);
		...
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.administratorsList());
		// ...generate the sub header...
		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-headSub");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyAdministrators_SubHead());

		// ...generate the main body of information...
		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-info");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyAdministrators_Info_1(m_company, m_product));

		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-info");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyAdministrators_Info_2(m_company, m_product));
	}
}
