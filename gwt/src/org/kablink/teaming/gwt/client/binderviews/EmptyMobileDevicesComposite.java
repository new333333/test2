...
/**
 * Class that holds the widget displayed for an empty mobile devices
 * view.
 * 
 * @author drfoster@novell.com
 */
public class EmptyMobileDevicesComposite extends ResizeComposite {
	private boolean						m_systemView;	// true -> Viewing the mobile devices system wide.  false -> Viewing the mobile devices for a specific user.
	 ...
	 * @param ct
	 */
	public EmptyMobileDevicesComposite(boolean systemView) {
	...
	private void createContent() {
		...
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.mobileDevicesList());

		// ...generate the sub header...
		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-headSub");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyMobileDevices_SubHead());
		...
		String txt;
		if (m_systemView)
		     txt = m_messages.emptyMobileDevices_Info_1_System(m_company, m_product);
		else txt = m_messages.emptyMobileDevices_Info_1_User(  m_company, m_product);
		fp.getElement().setInnerText(txt);

		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-info");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyMobileDevices_Info_2(m_company, m_product));
	}
}
