...
tinyMCE.PluginManager.requireLangPack('ss_addimage', 'en,tr,de,sv,zh_cn,cs,fa,fr_ca,fr,pl,pt_br,nl,he,nb,ru,ru_KOI8-R,ru_UTF-8,nn,cy,es,is,zh_tw,zh_tw_utf8,sk,da');

var TinyMCE_ssAddImagePlugin = {
	getInfo : function() {
		return {
			longname : 'Advanced image',
			author : 'Moxiecode Systems',
			authorurl : 'http://tinymce.moxiecode.com',
			infourl : 'http://tinymce.moxiecode.com/tinymce/docs/plugin_ss_addimage.html',
	...
	execCommand : function(editor_id, element, command, user_interface, value) {
		switch (command) {
			case "mce_ssAddImage":
				...
				template['file']   = '../../plugins/ss_addimage/image.htm';
				template['width']  = 480;
				template['height'] = 380;

				// Language specific width and height addons
				template['width']  += tinyMCE.getLang('lang_ss_addimage_delta_width', 0);
				template['height'] += tinyMCE.getLang('lang_ss_addimage_delta_height', 0);
	...
	handleNodeChange : function(editor_id, node, undo_index, undo_levels, visual_aid, any_selection) {
		...
		do {
			if (node.nodeName == "IMG" && tinyMCE.getAttrib(node, 'class').indexOf('mceItem') == -1) {
				tinyMCE.switchClass(editor_id + '_ss_addimage', 'mceButtonSelected');
				return true;
			}
		} while ((node = node.parentNode));

		tinyMCE.switchClass(editor_id + '_ss_addimage', 'mceButtonNormal');
...
tinyMCE.addPlugin("ss_addimage", TinyMCE_ssAddImagePlugin);
