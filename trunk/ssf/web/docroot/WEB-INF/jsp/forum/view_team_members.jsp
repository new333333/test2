...
<ssf:toolbar toolbar="${ssFolderToolbar}" style="ss_actions_bar" />
					...
					<img border="0" src="<html:imagesPath/>icons/group.gif"/> <span class="ss_largerprint ss_bold">Group members</span> <span class="ss_fineprint ss_light">Number of Team members</span> <span class="ss_fineprint ss_bold">${ssTeamMembersCount}</span>
					</div>
					...
					<c:forEach var="member" items="${ssTeamMembers}">
						...
						<tr class="options">
							<td class="ss_light ss_fineprint selectall"><input type="checkbox" id="team_member_all_ids" onchange="selectUnselectSynchronizedCheckboxes('team_member_all_ids', 'team_member_ids');" /><br/><label for="team_member_all_ids">Select all</label></td>
							<td colspan="5"></td>
						</tr>					
					</table>
						
					<div class="ss_buddiesListFooter">
						<!-- <a href=""><img border="0" src="<html:imagesPath/>icons/cancel_mail.gif" style="vertical-align: text-bottom; "/> Cancel</a> -->
						<a href="${send_mail_url}" onClick="appendUserIdsToURL(this); ss_toolbarPopupUrl(this.href);return false;"><img border="0" src="<html:imagesPath/>icons/send_mail.gif" style="vertical-align: text-bottom; "/> Send Mail</a>
					</div>
