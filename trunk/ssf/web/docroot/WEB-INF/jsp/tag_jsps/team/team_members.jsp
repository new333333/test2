...
<div class="teamIcon">
	<img id="teamIcon_${prefix}" src="<html:imagesPath/>icons/group.gif" onmouseover="displayTeamMembersMenu('<ssf:url adapter="true" portletName="ss_forum" action="__ajax_request" actionUrl="true"><ssf:param name="operation" value="get_team_members_count" /><ssf:param name="binderId" value="${binderId}" /></ssf:url>', '${prefix}');" />
	<div id="teamMenu_${prefix}" class="teamIconMenuPane" style="visibility: hidden; display: none; position: absolute;">
		<ul id="teamMembersListUL_${prefix}" class="ss_finestprint">
			<li class="pasteAllUsers" onmouseover="this.style.backgroundColor='#333'; this.style.color='#FFF'; " onmouseout="this.style.backgroundColor='#FFF'; this.style.color='#333';" onclick="loadTeamMembers('<ssf:url adapter="true" portletName="ss_forum" action="__ajax_request" actionUrl="true"><ssf:param name="operation" value="get_team_members" /><ssf:param name="binderId" value="${binderId}" /></ssf:url>', '${prefix}', '${clickRoutine}', addAllUsersFromTeam);"><ssf:nlt tag="teamMembers.addAll"/> (<strong id="teamMembersAmount_${prefix}">${teamMembersCount}</strong>)</li>	
		</ul>
	</div>	
</div>
