...
/**
 * Team Workspaces (root binder) view.
 * 
...
public class TeamWorkspacesView extends DataTableFolderViewBase {
	 ...
	 */
	private TeamWorkspacesView(BinderInfo folderInfo, UIObject parent, ViewReady viewReady) {
		// Simply initialize the base class.
		super(folderInfo, parent, viewReady, "vibe-teamWorkspacesDataTable");
	}
	
	/**
	 * Resets the columns as appropriate for the team workspaces view.
	 * 
	...
	public void constructView() {
		// Setup the appropriate styles for a team workspaces binder,
		// populate the view's contents and tell the base class that
		// we're done with the construction.
		getFlowPanel().addStyleName("vibe-teamWorkspacesFlowPanel");
		viewReady();
		populateContent();
	}
	
	/**
	 * Loads the TeamWorkspacesView split point and returns an
	...
	public static void createAsync(final BinderInfo folderInfo, final UIObject parent, final ViewReady viewReady, final ViewClient vClient) {
		GWT.runAsync(TeamWorkspacesView.class, new RunAsyncCallback() {			
			@Override
			public void onSuccess() {
				TeamWorkspacesView dfView = new TeamWorkspacesView(folderInfo, parent, viewReady);
				vClient.onSuccess(dfView);
			}
			
			@Override
			public void onFailure(Throwable reason) {
				Window.alert(m_messages.codeSplitFailure_TeamWorkspacesView());
	...
	protected boolean includePanel(FolderPanels folderPanel) {
		// In the team workspaces view, we don't show the filter panel
		// beyond the default.
		boolean reply;
		switch (folderPanel) {
	...
	/**
	 * Called from the base class to resize the content of this team
	 * workspaces view.
	 * 
