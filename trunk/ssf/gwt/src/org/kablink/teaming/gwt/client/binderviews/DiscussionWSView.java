...
public class DiscussionWSView extends ViewBase implements ToolPanelReady
{
	...
	private DiscussionWSView( BinderInfo binderInfo, ViewReady viewReady )
	{
		...
		// Build a skeleton of this view that will be filled in later.
		buildViewSkeleton();
	...
	/**
	 * Build a skeleton of this view that will be filled in later.  This view will have
	 ...
	 * 6. Footer
	 */
	private void buildViewSkeleton()
	{
		...
		// if there is one.
		{
			...
			LandingPage.createAsync( m_binderInfo, null, false, false, new ViewClient()
			{
				@Override
				public void onUnavailable()
				{
	...
	/**
	 * Issue an ajax call to read the necessary data for this discussion workspace.
	 */
	private void getWorkspaceData()
	{
		...
		if ( null == m_binderInfo )
			...
			binderId = m_binderInfo.getBinderId();
		...
		GwtClientHelper.executeCommand( cmd, new AsyncCallback<VibeRpcResponse>()
		{
			...
			public void onSuccess( VibeRpcResponse response )
			{
				...
				cmd = new Scheduler.ScheduledCommand()
				{
					public void execute()
					{
						// Update the view with the information we retrieved.
