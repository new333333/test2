...
public class FileFolderView extends DataTableFolderViewBase {
	...
	protected Widget getEmptyTableWidget() {
		Widget reply;
		FolderDisplayDataRpcResponseData fdd = getFolderDisplayData();
		if (fdd.getFolderOwnedByCurrentUser()) {
			if (fdd.getFolderIsMyFilesStorage())
			     reply = new EmptyMyFilesStorageComposite();
			else reply = new EmptyFileFolderComposite();
		}
		else {
