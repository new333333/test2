 ...
 * 20120303 (DRF):
 * 
 * This is a patched version of Google's MenuBar class.  The patch
 * involves a the popup menu is positioned when against the right
 * edge of the screen.
 * 
 * See the method setPosition() within the callback of
	...
	public void moveSelectionDown() {
		...
		if (vertical) {
			...
			if (selectedItem.getSubMenu() != null
					...
					&& (shownChildMenu == null || shownChildMenu
				...
				if (LocaleInfo.getCurrentLocale().isRTL()) {
					if (vertical) {
						popup.setPopupPosition(MenuBar.this.getAbsoluteLeft()
								- offsetWidth + 1, item.getAbsoluteTop());
					} else {
