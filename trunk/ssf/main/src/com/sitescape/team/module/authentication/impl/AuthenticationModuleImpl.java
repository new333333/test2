...
public class AuthenticationModuleImpl extends CommonDependencyInjection
	...
	{
		AuthenticationConfig authConfig = (AuthenticationConfig) getCoreDao().load(AuthenticationConfig.class, zoneId);
		if(authConfig == null) {
			// This should only happen in development, immediately after revision 9000
			authConfig = new AuthenticationConfig();
			authConfig.setZoneId(zoneId);
			getCoreDao().save(authConfig);
		}
		return authConfig;
	}
}