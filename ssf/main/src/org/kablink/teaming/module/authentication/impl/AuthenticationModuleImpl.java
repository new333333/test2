...
import org.kablink.teaming.spring.security.userdetails.OpenIDAuthenticationUserDetailsService;
...
public class AuthenticationModuleImpl extends BaseAuthenticationModule
	...
	protected Map<Long, ProviderManager> ldapAuthenticators = null;
	...
	// Unlike LDAP and local authenticators, for OpenID we use a single global
	// authentication provider across all zones.
	protected OpenIDAuthenticationProvider openidAuthenticationProvider = null;
	...
	public AuthenticationModuleImpl() throws ClassNotFoundException {
		ldapAuthenticators = new HashMap<Long, ProviderManager>();
		localProviders = new HashMap<Long, ZoneAwareLocalAuthenticationProvider>();
		lastUpdates = new ConcurrentHashMap<Long, Long>();
	}

	public void afterPropertiesSet() throws Exception {
		localAuthenticationProviderClass = ReflectHelper
				.classForName(SPropsUtil
						.getString("local.authentication.provider.class",
								"org.kablink.teaming.spring.security.ZoneAwareLocalAuthenticationProviderImpl"));
		authenticateLdapMatchingUsersUsingLdapOnly = SPropsUtil.getBoolean(
				"authenticate.ldap.matching.users.using.ldap.only", true);
		createOpenIDAuthenticationProvider();
	}

	protected void createOpenIDAuthenticationProvider() throws Exception {
		openidAuthenticationProvider = new OpenIDAuthenticationProvider();
		openidAuthenticationProvider.setCoreDao(getCoreDao());
		openidAuthenticationProvider.setZoneModule(getZoneModule());
	...
	protected void addZone(ZoneConfig zoneConfig) throws Exception {
		String zoneName = getZoneModule().getZoneInfo(zoneConfig.getZoneId())
				.getZoneName();
		if (ldapAuthenticators.containsKey(zoneConfig.getZoneId())) {
			logger.error("Duplicate zone added to AuthenticationModule: "
					+ zoneConfig.getZoneId() + " " + zoneName);
		...
		ldapAuthenticators.put(zoneConfig.getZoneId(), null);

		rebuildProvidersForZone(zoneConfig);
	}

	protected ZoneAwareLocalAuthenticationProvider newZoneAwareLocalAuthenticationProviderInstance(
			String zoneName) throws InstantiationException,
			IllegalAccessException, ClassNotFoundException {
		ZoneAwareLocalAuthenticationProvider provider = (ZoneAwareLocalAuthenticationProvider) localAuthenticationProviderClass
				.newInstance();
	...
	protected void removeZone(Long zoneId) {
		if (ldapAuthenticators.containsKey(zoneId)) {
			ldapAuthenticators.remove(zoneId);
	...
	protected void ensureZoneIsConfigured(Long zoneId) throws Exception {
		try {
			...
			AuthenticationConfig authConfig = zoneConfig
					.getAuthenticationConfig();
			Long lastUpdateInDb = authConfig.getLastUpdate();
			Long lastUpdateInMem = lastUpdates.get(zoneId);
			if ((lastUpdateInDb != null)
					&& ((lastUpdateInMem == null) || (lastUpdateInDb
							.compareTo(lastUpdateInMem) > 0))) {
				try {
					rebuildProvidersForZone(zoneConfig);
				} catch (Exception e) {
					logger.error("Unable to rebuild providers for zone "
							+ zoneId + ": " + e.toString());
				}
			}
	...
	protected void rebuildProvidersForZone(ZoneConfig zoneConfig)
			throws Exception {
		ProviderManager pm = ldapAuthenticators.get(zoneConfig.getZoneId());
		List<AuthenticationProvider> providers = createProvidersForZone(zoneConfig
				.getZoneId());
		if (providers.size() > 0) { // we've got external authenticators such as
									// LDAP
			if (pm != null) {
				...
				pm.setProviders(providers);
				ldapAuthenticators.put(zoneConfig.getZoneId(), pm);
			}
		} else { // no external authenticators
			if (pm != null)
				ldapAuthenticators.put(zoneConfig.getZoneId(), null);
		}
		lastUpdates.put(zoneConfig.getZoneId(), zoneConfig
				.getAuthenticationConfig().getLastUpdate());
	}
	...
	protected List<AuthenticationProvider> createProvidersForZone(Long zoneId)
		...
		for (LdapConnectionConfig config : getLdapConnectionConfigs(zoneId)) {
			...
			if (config.getUserSearches().size() > 0) {
				...
				try {
					...
					// The call to new DefaultSpringSecurityContextSource() will
					// fail if
					...
					if (url != null) {
						...
						// ldap authentication fails if the word "ldap" in the
						// url is not all lower case. Bug 553190
					...
					// Set the property that tells ldap whether or not to
					// dereference aliases.
					derefAliases = SPropsUtil.getString(
					...
					logger.warn("Unable to create LDAP context for url "
							+ config.getUrl() + ": " + e.toString());
					continue;
				}
				if (Validator.isNotNull(config.getPrincipal())) {
					contextSource.setUserDn(config.getPrincipal());
					contextSource
							.setPassword(config.getCredentials() == null ? ""
				...
				// Get the mapping of ldap attribute names to Teaming attribute
				// names.
				attributeMappings = config.getMappings();

				// Is the name of the ldap attribute that holds the ldap guid
				// defined?
				ldapGuidAttributeName = config.getLdapGuidAttribute();
				if (ldapGuidAttributeName != null
						&& ldapGuidAttributeName.length() > 0) {
					// Yes, map the name of the ldap attribute to the Teaming
					// attribute, "ldapGuid"
					attributeMappings.put(ldapGuidAttributeName, "ldapGuid");
				}

				SsfContextMapper contextMapper = new SsfContextMapper(
						getZoneModule(), attributeMappings);

				for (LdapConnectionConfig.SearchInfo us : config
						.getUserSearches()) {
					providers.add(createLdapAuthenticationProvider(
							contextSource, contextMapper, us, search));
					providers
							.add(createPreAuthenticatedLdapAuthenticationProvider(
									contextSource, contextMapper, us, search));
				}
			}
		}
		 ...
		 * providers.add(SpringAuthenticationBeans
		 * .getInstance().findProvider(zoneInfo.getZoneId(), "ldapTwo")); } else
		 * { providers.add(SpringAuthenticationBeans.getInstance().findProvider(
		 * zoneInfo.getZoneId(), "dynamicLdap")); }
		 */
		return providers;
	}
	...
	protected AuthenticationProvider createLdapAuthenticationProvider(
			...
			SsfContextMapper contextMapper, LdapConnectionConfig.SearchInfo us,
			String search) {
	 ...
	 * {@link org.springframework.security.core.AuthenticationManager#authenticate(Authentication)}
	 * . Delegates the authentication to the AuthenticationManager configured
	 * for the zone to which the request was directed.
	 * 
	...
	public Authentication authenticate(Authentication authentication)
		...
		try {
			// The following hack(?) is necessary to handle the
			// pre-authentication situation where
			// initial authentication request is made in the context of no user,
			// yet at least one
			// of the interceptors associated with this method invocation relies
			// on such context
			// being there. To work around that, we simply set up guest context
			// and leave it until
			...
			if (RequestContextHolder.getRequestContext() == null) {
				...
				RequestContextHolder.setRequestContext(new RequestContext(
						zoneName, guestUserName, null).resolve());
			}
			...
			if (!enable) {
				if (logger.isDebugEnabled())
					logger.debug("Rejecting " + getAuthenticator()
							+ " authentication request from "
							+ authentication.getName() + ": It is disabled");
			...
			try {
				...
				try {
					...
					SimpleProfiler
							.start("1-AuthenticationModuleImpl.doAuthenticate()");
					Authentication retVal = doAuthenticate(authentication);
					SimpleProfiler
							.stop("1-AuthenticationModuleImpl.doAuthenticate()");
				...
				Long zone = getZoneModule().getZoneIdByVirtualHost(
						ZoneContextHolder.getServerName());
				logger.warn("Authentication failure for zone " + zone + ": "
						+ e.toString());
				throw e;
			...
			if (logger.isDebugEnabled())
				logger.debug("Authenticating '" + authentication.getName()
						+ "' - " + ((System.nanoTime() - begin) / 1000000.0)
						+ " ms");
		}
	}
	...
	protected Authentication doAuthenticate(Authentication authentication)
			throws AuthenticationException {
		if(isExternalUserAuthentication(authentication))
			return doAuthenticateExternalUser(authentication);
		else
			return doAuthenticateInternalUser(authentication);
	}
	
	protected boolean isExternalUserAuthentication(Authentication authentication) {
		return (authentication instanceof OpenIDAuthenticationToken);
	}
	
	protected Authentication doAuthenticateInternalUser(Authentication authentication)
			throws AuthenticationException {
		...
		try {
			SimpleProfiler
					.start("2-AuthenticationModuleImpl.ensureZoneIsConfigured()");
			ensureZoneIsConfigured(zone);
			SimpleProfiler
					.stop("2-AuthenticationModuleImpl.ensureZoneIsConfigured()");
		} catch (Exception e) {
			logger.error("Unable to configure authentication for zone " + zone,
					e);
			throw new AuthenticationServiceException(
					"Unable to configure authentication for zone " + zone, e);
		}

		if (ldapAuthenticators.containsKey(zone)) {
			// Don't allow anyone to log in without specifying a password (to
			// prevent successful
			// authentication on an account that has no password).
			if (authentication.getCredentials() == null
					|| authentication.getCredentials().equals(""))
				throw new BadCredentialsException("Password is required");

			Authentication result = null;

			// Are we dealing with one of Teaming's system accounts such as
			// "admin" or "guest"?
			if (!MiscUtil.isSystemUserAccount(authentication.getName())) {
				// No, try to do an ldap authentication.
				// This will also try local authentication as fallback, if
				// configured to do so.
				try {
					// Perform authentication
					SimpleProfiler
							.start("3-authenticators.get(zone).authenticate(authentication)");
					result = performAuthentication(zone, authentication);
					SimpleProfiler
							.stop("3-authenticators.get(zone).authenticate(authentication)");
					...
					if (result instanceof PreAuthenticatedAuthenticationToken) {
						...
						loginName = WindowsUtil
								.getSamaccountname((String) result.getName());
					} else {
						loginName = (String) result.getName();
					}

					if (SPropsUtil.getBoolean("authenticator.synch."
							+ getAuthenticator(), false)) {
						// This is not used for authentication but for
						// synchronization.
						SimpleProfiler
						...
						// This is not used for authentication or
					...
					// This exception means that there are multiple users in the
					// ldap directory
					// that have the same name as the user id supplied by the
					// user.
					errDesc = NLT.get(
							"errorcode.login.failed.loginNameNotUnique", args);

					exc = new UserIdNotUniqueException(errDesc);
				}
			} else { // Yes, system account
						// Do not try ldap authentication. Authenticate against
						// Teaming db.
				SimpleProfiler
						.start("3a-system account: localProviders.get(zone).authenticate(authentication)");
				result = localProviders.get(zone).authenticate(authentication);
				SimpleProfiler
						.stop("3a-system account: localProviders.get(zone).authenticate(authentication)");

				// This is not used for authentication or synchronization but
				// merely to log the authenticator.
				SimpleProfiler
						.start("4a-system account: AuthenticationManagerUtil.authenticate()");
				AuthenticationManagerUtil.authenticate(
						getZoneModule().getZoneNameByVirtualHost(
								ZoneContextHolder.getServerName()),
						(String) result.getName(),
						(String) result.getCredentials(), false, false, true,
						(Map) result.getPrincipal(), getAuthenticator());
				SimpleProfiler
						.stop("4a-system account: AuthenticationManagerUtil.authenticate()");
	...
	protected Authentication doAuthenticateExternalUser(Authentication authentication) {
		return null;
	}
	 ...
	 * supports the indicated <Code>Authentication</code> object. Delegates the
	 * decision to the providers of the AuthenticationManager configured for the
	 * zone to which the request was directed.
	 * <p>
	...
	public boolean supports(Class authentication) {
		...
		if(openidAuthenticationProvider.supports(authentication))
			return true;
		
		// Move on to checking zone-specific providers.
		Long zone = getZoneModule().getZoneIdByVirtualHost(
				ZoneContextHolder.getServerName());
		try {
			ensureZoneIsConfigured(zone);
		...
		if (ldapAuthenticators.containsKey(zone)) {
			ProviderManager pm = ldapAuthenticators.get(zone);
			if (pm != null) {
	...
	private Authentication performAuthentication(Long zoneId,
			Authentication authentication) throws AuthenticationException {
		AuthenticationException exc = null;
		ProviderManager pm = ldapAuthenticators.get(zoneId);
		...
		if (pm != null) {
			try {
				...
				if (logger.isDebugEnabled())
					logger.debug("External authentication failed: "
							+ e.toString());
				exc = e;
				if (authenticateLdapMatchingUsersUsingLdapOnly
