...
/**
 * Class that holds the widget displayed for an empty proxy identities
 * view.
 * 
 * @author drfoster@novell.com
 */
public class EmptyProxyIdentitiesComposite extends ResizeComposite {
	 ...
	 * @param ct
	 */
	public EmptyProxyIdentitiesComposite() {
	...
	private void createContent() {
		...
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.proxyIdentitiesList());

		// ...generate the sub header...
		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-headSub");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyProxyIdentities_SubHead());

		// ...generate the main body of information...
		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-info");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyProxyIdentities_Info_1(m_company, m_product));

		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-info");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyProxyIdentities_Info_2(m_company, m_product));
	}
}
