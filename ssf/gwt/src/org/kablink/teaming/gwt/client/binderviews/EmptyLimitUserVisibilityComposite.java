...
/**
 * Class that holds the widget displayed for an empty limit user
 * visibility view.
 * 
 * @author drfoster@novell.com
 */
public class EmptyLimitUserVisibilityComposite extends ResizeComposite {
	 ...
	 * @param ct
	 */
	public EmptyLimitUserVisibilityComposite() {
	...
	private void createContent() {
		...
		m_rootPanel.add(fp);
		Image i = GwtClientHelper.buildImage(m_filrImages.people_transparent_72().getSafeUri().asString());
		fp.add(i);
		...
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.limitUserVisibilityList());
		// ...generate the sub header...
		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-headSub");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyLimitUserVisibility_SubHead());

		// ...generate the main body of information...
		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-info");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyLimitUserVisibility_Info_1(m_company, m_product));

		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-info");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyLimitUserVisibility_Info_2(m_company, m_product));
	}
}
