...
public class DefinitionModuleImplTest extends AbstractTestBase {
	...
	public void addDefinitionNoName() throws Exception {
		...
		Definition d = definitions.addDefinition(IOUtils.toInputStream(getXml()),
				true);
		assertNotNull(d);
	...
	public void addDefinitionWithBinderNoName() throws Exception {
		...
		Definition d = definitions.addDefinition(IOUtils.toInputStream(getXml()), b,
				true);
		assertNotNull(d);
		assertEquals(name, d.getName());
	...
	public void getDefinitionAsXmlHasSchemaLocation() throws Exception {
		...
		Definition d = definitions.getDefinitionByName(name);
		QName schemaLocAttr = new QName("schemaLocation", new Namespace("xsi",
				"http://www.w3.org/2001/XMLSchema-instance"));
		assertFalse(maybe(
				d.getDefinition().getRootElement()
						.attributeValue(schemaLocAttr)).and(
		...
		assertTrue(maybe(
				definitions.getDefinitionAsXml(d).getRootElement()
						.attributeValue(schemaLocAttr)).and(
				new Func1<String, Boolean>() {
					public Boolean apply(String x) {
						// regexp below is approximate
