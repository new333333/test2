...
<c:if test="${ss_isBinderAdmin}">
<script type="text/javascript" src="<html:rootPath />js/jsp/tag_jsps/find/find.js"></script>
<%
	String propertyName = (String) request.getAttribute("property_name");
	java.util.List<Principal> groupList = new java.util.ArrayList();
...
<c:if test="${! empty ssDefinitionEntry}">
  ...
  <jsp:useBean id="grouplist_entry" type="org.kablink.teaming.domain.Principal" />
<%
	groupList = grouplist_entry.getMemberOf();
	if(groupList != null) {
		for (Principal group : groupList) {
			if (!group.getIdentityInfo().isFromLdap()) {
				groupListSet.add(group);
			}
		}
	}
%>
