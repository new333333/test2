...
/**
 * Global Workspaces (root binder) view.
 * 
 * @author drfoster@novell.com
 */
public class GlobalWorkspacesView extends DataTableFolderViewBase {
	/*
	 * Class constructor.
	 * 
	 ...
	 */
	private GlobalWorkspacesView(BinderInfo folderInfo, UIObject parent, ViewReady viewReady) {
		// Simply initialize the base class.
		super(folderInfo, parent, viewReady, "vibe-globalWorkspacesDataTable");
	}
	
	/**
	 * Resets the columns as appropriate for the global workspaces view.
	 * 
	...
	public void constructView() {
		// Setup the appropriate styles for a global workspaces binder,
		// populate the view's contents and tell the base class that
		// we're done with the construction.
		getFlowPanel().addStyleName("vibe-globalWorkspacesFlowPanel");
		viewReady();
		populateContent();
	}
	
	/**
	 * Loads the GlobalWorkspacesView split point and returns an
	...
	public static void createAsync(final BinderInfo folderInfo, final UIObject parent, final ViewReady viewReady, final ViewClient vClient) {
		GWT.runAsync(GlobalWorkspacesView.class, new RunAsyncCallback() {			
			@Override
			public void onSuccess() {
				GlobalWorkspacesView dfView = new GlobalWorkspacesView(folderInfo, parent, viewReady);
				vClient.onSuccess(dfView);
			}
			
			@Override
			public void onFailure(Throwable reason) {
				Window.alert(m_messages.codeSplitFailure_GlobalWorkspacesView());
	...
	protected Widget getEmptyTableWidget() {
		return new EmptyGlobalsComposite();
	}
	...
	protected boolean includePanel(FolderPanels folderPanel) {
		// In the global workspaces view, we don't show the filter
	...
	/**
	 * Called from the base class to reset the content of this global
	 * workspaces view.
	 * 
	...
	/**
	 * Called from the base class to resize the content of this global
	 * workspaces view.
	 * 
