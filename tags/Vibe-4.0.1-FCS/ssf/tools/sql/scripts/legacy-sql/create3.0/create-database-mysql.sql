...
create table SS_Definitions (id char(32) not null, lockVersion bigint not null, zoneId bigint not null, name varchar(64), title varchar(128), type integer, internalId varchar(32), visibility integer, encoding longblob, binderId bigint, creation_date datetime, creation_principal bigint, modification_date datetime, modification_principal bigint, primary key (id), unique (zoneId, name, binderId)) ENGINE=InnoDB;
...
create table SS_Events (id char(32) not null, lockVersion bigint not null, zoneId bigint, folderEntry bigint, principal bigint, binder bigint, ownerType varchar(16), ownerId bigint, owningBinderKey varchar(255), owningBinderId bigint, name varchar(64), dtStart datetime not null, dtEnd datetime, frequency integer, repeatInterval integer, until datetime, repeatCount integer, weekStart integer, timeZoneSensitive bit, days varchar(100), monthDay varchar(100), yearDay varchar(100), weekNo varchar(100), months varchar(100), minutes varchar(100), hours varchar(100), duration varchar(32) not null, creation_date datetime, creation_principal bigint, modification_date datetime, modification_principal bigint, timeZone varchar(80), calUid varchar(255), freeBusy varchar(32), primary key (id)) ENGINE=InnoDB;
...
create table SS_LdapConnectionConfig (id char(32) not null, zoneId bigint, url varchar(255), userIdAttribute varchar(255), mappings text, userSearches text, groupSearches text, principal varchar(255), credentials varchar(255), position integer, ldapGuidAttribute varchar(255), primary key (id)) ENGINE=InnoDB;
...
create table SS_TokenInfo (id char(32) not null, type char(1) not null, zoneId bigint, seed varchar(128), lastAccessTime datetime, userId bigint, applicationId bigint, binderId bigint, binderAccessConstraints integer, clientAddr varchar(128), primary key (id)) ENGINE=InnoDB;
create table SS_UserProperties (principalId bigint not null, binderId bigint not null, zoneId bigint, properties mediumblob not null, primary key (principalId, binderId)) ENGINE=InnoDB;
...
create table SS_ZoneConfig (zoneId bigint not null, upgradeVersion integer, mobileAccessEnabled bit, diskQuotasEnabled bit, diskQuotaUserDefault integer, diskQuotasHighwaterPercentage integer, postingEnabled bit, simpleUrlPostingEnabled bit, sendMailEnabled bit, defaultHomePageId bigint, defaultGuestHomePageId bigint, allowLocalLogin bit, allowAnonymousAccess bit, allowSelfRegistration bit, lastUpdate bigint, primary key (zoneId)) ENGINE=InnoDB;
...
create table JBPM_LOG (ID_ bigint not null auto_increment, CLASS_ char(1) not null, INDEX_ integer, DATE_ datetime, TOKEN_ bigint, PARENT_ bigint, SWIMLANEINSTANCE_ bigint, TASKINSTANCE_ bigint, TRANSITION_ bigint, TASKACTORID_ varchar(255), VARIABLEINSTANCE_ bigint, CHILD_ bigint, SOURCENODE_ bigint, DESTINATIONNODE_ bigint, TASKOLDACTORID_ varchar(255), MESSAGE_ varchar(4000), NODE_ bigint, ENTER_ datetime, LEAVE_ datetime, DURATION_ bigint, NEWLONGVALUE_ bigint, EXCEPTION_ varchar(4000), ACTION_ bigint, OLDBYTEARRAY_ bigint, NEWBYTEARRAY_ bigint, OLDSTRINGIDCLASS_ varchar(255), OLDSTRINGIDVALUE_ varchar(255), NEWSTRINGIDCLASS_ varchar(255), NEWSTRINGIDVALUE_ varchar(255), OLDDOUBLEVALUE_ double precision, NEWDOUBLEVALUE_ double precision, OLDSTRINGVALUE_ varchar(4000), NEWSTRINGVALUE_ varchar(4000), OLDDATEVALUE_ datetime, NEWDATEVALUE_ datetime, OLDLONGVALUE_ bigint, OLDLONGIDCLASS_ varchar(255), OLDLONGIDVALUE_ bigint, NEWLONGIDCLASS_ varchar(255), NEWLONGIDVALUE_ bigint, primary key (ID_)) ENGINE=InnoDB;
...
CREATE TABLE SSQRTZ_JOB_LISTENERS (JOB_NAME VARCHAR(200) NOT NULL,JOB_GROUP VARCHAR(200) NOT NULL,JOB_LISTENER VARCHAR(200) NOT NULL,PRIMARY KEY (JOB_NAME,JOB_GROUP,JOB_LISTENER),INDEX (JOB_NAME, JOB_GROUP))ENGINE=InnoDB;
...
CREATE TABLE SSQRTZ_SIMPLE_TRIGGERS (TRIGGER_NAME VARCHAR(200) NOT NULL,TRIGGER_GROUP VARCHAR(200) NOT NULL,REPEAT_COUNT BIGINT(7) NOT NULL,REPEAT_INTERVAL BIGINT(12) NOT NULL,TIMES_TRIGGERED BIGINT(10) NOT NULL,PRIMARY KEY (TRIGGER_NAME,TRIGGER_GROUP),INDEX (TRIGGER_NAME, TRIGGER_GROUP))ENGINE=InnoDB;
CREATE TABLE SSQRTZ_CRON_TRIGGERS (TRIGGER_NAME VARCHAR(200) NOT NULL,TRIGGER_GROUP VARCHAR(200) NOT NULL,CRON_EXPRESSION VARCHAR(120) NOT NULL,TIME_ZONE_ID VARCHAR(80),PRIMARY KEY (TRIGGER_NAME,TRIGGER_GROUP),INDEX (TRIGGER_NAME, TRIGGER_GROUP))ENGINE=InnoDB;
CREATE TABLE SSQRTZ_BLOB_TRIGGERS (TRIGGER_NAME VARCHAR(200) NOT NULL,TRIGGER_GROUP VARCHAR(200) NOT NULL,BLOB_DATA BLOB NULL,PRIMARY KEY (TRIGGER_NAME,TRIGGER_GROUP),INDEX (TRIGGER_NAME, TRIGGER_GROUP))ENGINE=InnoDB;
CREATE TABLE SSQRTZ_TRIGGER_LISTENERS (TRIGGER_NAME VARCHAR(200) NOT NULL,TRIGGER_GROUP VARCHAR(200) NOT NULL,TRIGGER_LISTENER VARCHAR(200) NOT NULL,PRIMARY KEY (TRIGGER_NAME,TRIGGER_GROUP,TRIGGER_LISTENER),INDEX (TRIGGER_NAME, TRIGGER_GROUP))ENGINE=InnoDB;
CREATE TABLE SSQRTZ_CALENDARS (CALENDAR_NAME VARCHAR(200) NOT NULL,CALENDAR BLOB NOT NULL,PRIMARY KEY (CALENDAR_NAME))ENGINE=InnoDB;
CREATE TABLE SSQRTZ_PAUSED_TRIGGER_GRPS (TRIGGER_GROUP VARCHAR(200) NOT NULL,PRIMARY KEY (TRIGGER_GROUP))ENGINE=InnoDB;
...
CREATE TABLE SSQRTZ_SCHEDULER_STATE (INSTANCE_NAME VARCHAR(200) NOT NULL,LAST_CHECKIN_TIME BIGINT(13) NOT NULL,CHECKIN_INTERVAL BIGINT(13) NOT NULL,PRIMARY KEY (INSTANCE_NAME))ENGINE=InnoDB;
