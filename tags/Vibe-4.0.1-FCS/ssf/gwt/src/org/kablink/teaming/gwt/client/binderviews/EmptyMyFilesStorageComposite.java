...
/**
 * Class that holds the widget displayed for an empty My Files Storage
 * view.
 * 
 * @author drfoster@novell.com
 */
public class EmptyMyFilesStorageComposite extends ResizeComposite {
	 ...
	 * @param ct
	 */
	public EmptyMyFilesStorageComposite() {
	...
	private void createContent() {
		...
		m_rootPanel.add(fp);
		Image i = GwtClientHelper.buildImage(m_filrImages.myFilesStorage_transparent_72().getSafeUri().asString());
		fp.add(i);
		...
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.myFilesStorage());

		// ...generate the sub header...
		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-headSub");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyMyFilesStorage_SubHead());

		// ...generate the main body of information...
		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-info");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyMyFilesStorage_Info_1());
		
		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-info");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyMyFilesStorage_Info_2());

		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-info");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyMyFilesStorage_Info_3(m_company, m_product));

		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-info");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyMyFilesStorage_Info_4(m_product));
	}
}
