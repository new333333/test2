...
public class NetworkUtil {
	...
	private static String _getLocalHostIPv4Address() {
		// First, check if specific IP address is assigned in configuration
		String address = SPropsUtil.getString("localhost.ipv4.address", "");
		if(Validator.isNull(address)) {
			// Get it from the runtime environment
			try {
				return java.net.InetAddress.getLocalHost().getHostAddress();
			} catch (UnknownHostException e) {
				return null; // This shouldn't happen in Vibe/Filr
			}
		}
		else {
			return address;
