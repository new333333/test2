...
/**
 * Class that holds the widget displayed for an empty file folder view.
 * 
 * @author drfoster@novell.com
 */
public class EmptyFileFolderComposite extends ResizeComposite {
	 ...
	 * @param ct
	 */
	public EmptyFileFolderComposite() {
	...
	private void createContent() {
		...
		m_rootPanel.add(fp);
		Image i = GwtClientHelper.buildImage(m_filrImages.fileFolder_transparent_72().getSafeUri().asString());
		fp.add(i);
		...
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.folders());

		// ...generate the sub header...
		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-headSub");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyFileFolder_SubHead());

		// ...generate the main body of information...
		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-info");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyFileFolder_Info_1());

		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-info");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyFileFolder_Info_2(m_company, m_product));

		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-info");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyFileFolder_Info_3(m_product));
	}
}
