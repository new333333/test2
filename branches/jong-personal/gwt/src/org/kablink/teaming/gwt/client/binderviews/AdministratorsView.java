...
/**
 * Administrators (profiles root binder) view.
 * 
 * @author drfoster@novell.com
 */
public class AdministratorsView extends DataTableFolderViewBase {
	/*
	 * Class constructor.
	 * 
	...
	/**
	 * Resets the columns as appropriate for the administrators view.
	 * 
	...
	public void constructView() {
		// Setup the appropriate styles for a administrators binder,
		// populate the view's contents and tell the base class that
		// we're done with the construction.
		getFlowPanel().addStyleName("vibe-administratorsFlowPanel");
		viewReady();
		populateContent();
	}
	
	/**
	 * Loads the AdministratorsView split point and returns an
	...
	public static void createAsync(final BinderInfo folderInfo, final ViewReady viewReady, final ViewClient vClient) {
		GWT.runAsync(AdministratorsView.class, new RunAsyncCallback() {			
			...
			public void onFailure(Throwable reason) {
				Window.alert(m_messages.codeSplitFailure_AdministratorsView());
	...
	protected Widget getEmptyTableWidget() {
		return new EmptyAdministratorsComposite();
	}
	...
	protected boolean includePanel(FolderPanels folderPanel) {
		// In the administrators view, we don't show the filter panel
		// beyond the default.
		boolean reply;
		switch (folderPanel) {
		...
		// console... 
		if (reply && getFolderInfo().getWorkspaceType().isAdministratorManagement()) {
