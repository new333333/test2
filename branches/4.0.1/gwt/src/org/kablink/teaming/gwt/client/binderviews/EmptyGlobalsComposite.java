...
/**
 * Class that holds the widget displayed for an empty global workspaces
 * view.
 * 
 * @author drfoster@novell.com
 */
public class EmptyGlobalsComposite extends ResizeComposite {
	...
	private void createContent() {
		...
		m_rootPanel.add(fp);
		Image i = GwtClientHelper.buildImage(m_filrImages.globals_transparent_72().getSafeUri().asString());
		fp.add(i);
		...
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.globalsList());

		// ...generate the sub header...
		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-headSub");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyGlobals_SubHead());

		// ...generate the main body of information...
		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-info");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyGlobals_Info_1(m_company, m_product));

		fp = new VibeFlowPanel();
		fp.addStyleName("vibe-emptyWidget-info");
		m_rootPanel.add(fp);
		fp.getElement().setInnerText(m_messages.emptyGlobals_Info_2(m_company, m_product));
	}
}
