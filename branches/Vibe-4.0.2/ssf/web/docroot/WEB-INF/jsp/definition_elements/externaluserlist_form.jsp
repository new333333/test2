...
<c:if test="${! empty ssDefinitionEntry}">
	<c:set var="externalUserList_entry" value="${ssDefinitionEntry}"/>
	<jsp:useBean id="externalUserList_entry" type="org.kablink.teaming.domain.DefinableEntity" />
	<%
		String propertyName = ((String) request.getAttribute("property_name"));
		if ((null != propertyName) && (0 < propertyName.length())) {
			CustomAttribute cAttr = externalUserList_entry.getCustomAttribute(propertyName);
			if (null != cAttr) {
				externalUserListSet = cAttr.getValueSet();
			}
		}
	%>
</c:if>
	...
	<div class="ss_entryContent">
		...
		<c:forEach var="externalUserItem" items="<%= externalUserListSet %>">
			<img
				...
				alt="<ssf:nlt tag="presence.none"/>"/> ${externalUserItem}<br/>
		</c:forEach>
	</div>
<% } %>
