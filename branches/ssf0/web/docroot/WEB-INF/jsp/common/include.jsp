...
<%
	...
	if (com.sitescape.team.context.request.RequestContextHolder
			.getRequestContext() != null) {
		com.sitescape.team.domain.User user = com.sitescape.team.context.request.RequestContextHolder
...
<ssf:ifadapter>
	<c:if test="${empty ssf_support_files_loaded}">
		...
		<c:if test="${empty ssf_snippet}">
			...
			<html>
			<head>
			<c:if test="${!empty ss_windowTitle}">
				<title>${ss_windowTitle}</title>
			</c:if>
		</c:if>
	</c:if>
