...
create table SS_Definitions (id char(32) not null, lockVersion bigint not null, name varchar(64), title varchar(128), type integer, internalId varchar(32), visibility integer, zoneId bigint not null, encoding mediumblob, creation_date datetime, creation_principal bigint, modification_date datetime, modification_principal bigint, primary key (id)) ENGINE=InnoDB;
create table SS_Events (id char(32) not null, lockVersion bigint not null, folderEntry bigint, principal bigint, binder bigint, ownerType varchar(16), ownerId bigint, owningBinderKey varchar(255), owningBinderId bigint, name varchar(64), dtStart datetime not null, dtEnd datetime, frequency integer, repeatInterval integer, until datetime, repeatCount integer, weekStart integer, timeZoneSensitive bit, days varchar(100), monthDay varchar(100), yearDay varchar(100), weekNo varchar(100), months varchar(100), minutes varchar(100), hours varchar(100), duration varchar(32) not null, creation_date datetime, creation_principal bigint, modification_date datetime, modification_principal bigint, timeZone varchar(16), primary key (id)) ENGINE=InnoDB;
create table SS_FolderEntries (id bigint not null auto_increment, lockVersion bigint not null, deleted bit, reserved_date datetime, reserved_principal bigint, entry_level integer, entry_sortKey varchar(255) unique, owningBinderKey varchar(255), lastActivity datetime, logVersion bigint, replyCount integer, nextDescendant integer, totalReplyCount integer, postedBy varchar(256), topEntry bigint, parentEntry bigint, wrk_date datetime, wrk_principal bigint, parentBinder bigint, description_text mediumtext, description_format integer, title varchar(255), normalTitle varchar(255), iconName varchar(64), entryDef char(32), creation_date datetime, creation_principal bigint, modification_date datetime, modification_principal bigint, definitionType integer, ratings_average double precision, ratings_count bigint, popularity bigint, primary key (id)) ENGINE=InnoDB;
...
create table SS_LibraryEntries (binderId bigint not null, type bigint not null, name varchar(255) not null, entityId bigint, primary key (binderId, type, name)) ENGINE=InnoDB;
...
create table SS_Postings (id char(32) not null, lockVersion bigint not null, enabled bit, emailAddress varchar(256), password varchar(64), zoneId bigint not null, binder bigint, definition char(32), replyPostingOption integer, primary key (id)) ENGINE=InnoDB;
...
create table SS_Principals (id bigint not null auto_increment, type varchar(16) not null, lockVersion bigint not null, workspaceId bigint, internalId varchar(32), deleted bit, disabled bit, zoneId bigint, logVersion bigint, emailAddress varchar(256), name varchar(128), foreignName varchar(128), theme varchar(64), parentBinder bigint, description_text mediumtext, description_format integer, title varchar(255), normalTitle varchar(255), iconName varchar(64), definitionType integer, entryDef char(32), creation_date datetime, creation_principal bigint, modification_date datetime, modification_principal bigint, displayStyle varchar(16), loginDate datetime, firstName varchar(64), lastName varchar(64), middleName varchar(64), organization varchar(256), phone varchar(128), zonName varchar(100), password varchar(64), timeZone varchar(80), locale varchar(32), digestSeed bigint, primary key (id), unique (zoneId, foreignName), unique (zoneId, name)) ENGINE=InnoDB;
create table SS_Ratings (principalId bigint not null, entityId bigint not null, entityType integer not null, rating bigint, readCount bigint, primary key (principalId, entityId, entityType)) ENGINE=InnoDB;
create table SS_SeenMap (principalId bigint not null, seenMap mediumblob not null, lastPrune datetime, primary key (principalId)) ENGINE=InnoDB;
create table SS_Subscriptions (principalId bigint not null, entityId bigint not null, entityType integer not null, style integer, primary key (principalId, entityId, entityType)) ENGINE=InnoDB;
create table SS_Tags (id char(32) not null, entity_id bigint, entity_type integer, owner_id bigint, owner_type integer, name varchar(64), isPublic bit, primary key (id)) ENGINE=InnoDB;
create table SS_UserProperties (principalId bigint not null, binderId bigint not null, properties blob not null, primary key (principalId, binderId)) ENGINE=InnoDB;
...
create table SS_WorkflowResponses (id char(32) not null, folderEntry bigint, principal bigint, binder bigint, ownerType varchar(16), ownerId bigint, owningBinderKey varchar(255), owningBinderId bigint, name varchar(64), response varchar(2000), definitionId varchar(32), responderId bigint, responseDate datetime, primary key (id)) ENGINE=InnoDB;
create table SS_WorkflowStates (tokenId bigint not null, lockVersion bigint not null, folderEntry bigint, principal bigint, binder bigint, ownerType varchar(16), ownerId bigint, owningBinderKey varchar(255), owningBinderId bigint, wrk_date datetime, wrk_principal bigint, state varchar(64), threadName varchar(64), timerId bigint, definition char(32), primary key (tokenId)) ENGINE=InnoDB;
...
alter table SS_Attachments add constraint FKA1AD4C3193118767 foreign key (creation_principal) references SS_Principals (id);
alter table SS_Attachments add constraint FKA1AD4C319C2BF2C9 foreign key (binder) references SS_Forums (id);
alter table SS_Attachments add constraint FKA1AD4C31DB0761E4 foreign key (modification_principal) references SS_Principals (id);
...
alter table SS_Attachments add constraint FKA1AD4C31F93A11B4 foreign key (filelock_owner) references SS_Principals (id);
...
alter table SS_Dashboards add constraint FKFA9653BE93118767 foreign key (creation_principal) references SS_Principals (id);
alter table SS_Dashboards add constraint FKFA9653BEDB0761E4 foreign key (modification_principal) references SS_Principals (id);
...
alter table SS_Definitions add constraint FK7B56F60193118767 foreign key (creation_principal) references SS_Principals (id);
alter table SS_Definitions add constraint FK7B56F601DB0761E4 foreign key (modification_principal) references SS_Principals (id);
...
alter table SS_Events add constraint FKDE0E53F893118767 foreign key (creation_principal) references SS_Principals (id);
alter table SS_Events add constraint FKDE0E53F89C2BF2C9 foreign key (binder) references SS_Forums (id) on delete cascade;
alter table SS_Events add constraint FKDE0E53F8DB0761E4 foreign key (modification_principal) references SS_Principals (id);
...
alter table SS_FolderEntries add constraint FKA6632C83F7719C70 foreign key (reserved_principal) references SS_Principals (id);
alter table SS_FolderEntries add constraint FKA6632C832592FE8C foreign key (topEntry) references SS_FolderEntries (id);
alter table SS_FolderEntries add constraint FKA6632C8393118767 foreign key (creation_principal) references SS_Principals (id);
alter table SS_FolderEntries add constraint FKA6632C83DB0761E4 foreign key (modification_principal) references SS_Principals (id);
...
alter table SS_FolderEntries add constraint FKA6632C83A3644438 foreign key (wrk_principal) references SS_Principals (id);
...
alter table SS_Forums add constraint FKDF668A5193118767 foreign key (creation_principal) references SS_Principals (id);
alter table SS_Forums add constraint FKDF668A51B356319A foreign key (posting) references SS_Postings (id);
alter table SS_Forums add constraint FKDF668A51DB0761E4 foreign key (modification_principal) references SS_Principals (id);
...
alter table SS_Notifications add constraint FK9131F9296EADA262 foreign key (principalId) references SS_Principals (id);
...
alter table SS_PrincipalMembership add constraint FK176F6225AEB5AABF foreign key (userId) references SS_Principals (id);
alter table SS_PrincipalMembership add constraint FK176F6225197D4C44 foreign key (groupId) references SS_Principals (id);
alter table SS_Principals add constraint FK7693816493118767 foreign key (creation_principal) references SS_Principals (id);
alter table SS_Principals add constraint FK76938164DB0761E4 foreign key (modification_principal) references SS_Principals (id);
...
alter table SS_WorkflowStates add constraint FK8FA8AA80A3644438 foreign key (wrk_principal) references SS_Principals (id);
...
create table JBPM_LOG (ID_ bigint not null auto_increment, CLASS_ char(1) not null, INDEX_ integer, DATE_ datetime, TOKEN_ bigint, PARENT_ bigint, SWIMLANEINSTANCE_ bigint, TASKINSTANCE_ bigint, EXCEPTION_ varchar(4000), ACTION_ bigint, TRANSITION_ bigint, SOURCENODE_ bigint, DESTINATIONNODE_ bigint, VARIABLEINSTANCE_ bigint, OLDLONGVALUE_ bigint, NEWLONGVALUE_ bigint, OLDDOUBLEVALUE_ double precision, NEWDOUBLEVALUE_ double precision, OLDSTRINGIDCLASS_ varchar(255), OLDSTRINGIDVALUE_ varchar(255), NEWSTRINGIDCLASS_ varchar(255), NEWSTRINGIDVALUE_ varchar(255), TASKOLDACTORID_ varchar(255), TASKACTORID_ varchar(255), OLDLONGIDCLASS_ varchar(255), OLDLONGIDVALUE_ bigint, NEWLONGIDCLASS_ varchar(255), NEWLONGIDVALUE_ bigint, NODE_ bigint, ENTER_ datetime, LEAVE_ datetime, DURATION_ bigint, MESSAGE_ varchar(4000), OLDBYTEARRAY_ bigint, NEWBYTEARRAY_ bigint, OLDDATEVALUE_ datetime, NEWDATEVALUE_ datetime, OLDSTRINGVALUE_ varchar(4000), NEWSTRINGVALUE_ varchar(4000), CHILD_ bigint, primary key (ID_)) ENGINE=InnoDB;
create table JBPM_MESSAGE (ID_ bigint not null auto_increment, CLASS_ char(1) not null, DESTINATION_ varchar(255), EXCEPTION_ varchar(4000), ISSUSPENDED_ bit, TOKEN_ bigint, TEXT_ varchar(255), NODE_ bigint, TRANSITIONNAME_ varchar(255), ACTION_ bigint, TASKINSTANCE_ bigint, primary key (ID_)) ENGINE=InnoDB;
