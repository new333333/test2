...
<c:if test="${ssEntry == ssDefinitionEntry}">
	<c:if test="${!empty ss_addReplyUrl}">
		<div style="padding-left: 5px; padding-top: 10px; padding-bottom: 10px;">
		  <a class="ss_tinyButton ss_fineprint ss_nowrap" href="${ss_addReplyUrl}"
		    onclick="ss_toolbarPopupUrl(this.href, 'blank', '', ''); return false;" 
			<ssf:title tag="entry.addComment.alt" />
		  >
			  <ssf:nlt tag="entry.addComment"/>
		  </a>
		</div>
	</c:if>
