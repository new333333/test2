...
public class ServerTaskLinkage {
	...
	private static void fixupTaskLinkList(List<ServerTaskLink> tlList, Map<Long, Long> entryIdMap) {
		...
		for (int i = (c - 1); i >= 0; i -= 1) {
			...
			if (null == newEntryId) {
				...
				m_logger.error("Task entry " + entryId + " missing from entry ID map, dropped from folder's linkage.");
				tlList.remove(i);
			}
			else {
	...
	/**
	 * Validates the task entry IDs referenced by a ServerTaskLinkage.
	 * 
	 * @param entryIds
	 */
